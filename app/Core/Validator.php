...
use App\Core\Libraries\MaxMind;
...
class Validator
{
    ...
    {
        return preg_match('/^(?!(?:(?:\x22?\x5C[\x00-\x7E]\x22?)|(?:\x22?[^\x5C\x22]\x22?)){255,
            ...
            \.(?:(?:25[0-5])|(?:2[0-4][0-9])|(?:1[0-9]{2})|(?:[1-9]?[0-9]))){3}))\]))$/', $email);
    }
    ...
    {
        return (!empty($password) && mb_strlen($password) >= 8);
    }

    /**
     * Check email and ip risk score
     *
     * @param array $params
     * @return bool
     */
    public static function validateRiskScore(array $params)
    {
        $formatedData = [
            'device' => [
        ...
        $scores = MaxMind::getScore($formatedData);

        return ($scores->ip_address->risk <= 10 && !$scores->email->is_high_risk);
    }
}